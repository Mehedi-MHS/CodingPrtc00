...
and the final value is initial(defaule)  value (optional)
-------------------------
...
function getRating(watchList) {
  ...
  let onlyChristopherMovies = watchList.filter(
    (movie) => movie.Director == "Christopher Nolan"
  );
  let ChristopherMovieRatingSum = onlyChristopherMovies.reduce(
    (sum, current) => sum + Number(current.imdbRating),
    0
  );
  let averageRating = ChristopherMovieRatingSum / onlyChristopherMovies.length;
